...
export default function Layout() {
  ...
  // تنظيف رقم الهاتف من الأحرف غير الرقمية للرابط المباشر
    ...
    <div className="min-h-screen flex flex-col bg-cream-50 text-navy-950 dark:bg-navy-950 dark:text-cream-50 transition-colors duration-300">
      ...
      {/*
        pb-[calc(env(safe-area-inset-bottom)+5rem)] :
          - 4rem  = ارتفاع MobileNav
          - 1rem  = مسافة إضافية فوقه
          - safe-area = يحترم الشاشات ذات الشريط السفلي (iPhone notch)
        lg:pb-0  : على الشاشات الكبيرة لا حاجة لهذا الـ padding
      */}
      <main className="flex-1 relative pb-[calc(env(safe-area-inset-bottom)+5rem)] lg:pb-0">
      ...
      {socialSettings?.showWhatsapp && cleanNumber && (
        ...
        >
          ...
          {/* نص منزلق عند hover — مخفي على الجوال */}
