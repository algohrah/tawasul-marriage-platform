...
const content = {
  privacy: {
    ...
    sections: [
      ...
      { title: '4. مشاركة المعلومات', body: 'لا نبيع بياناتك لأي طرف ثالث. قد نشارك معلومات محدودة مع مزوّدي الخدمات الموثوقين الذين يساعدون في تشقيل المنصة، وذلك تحت اتفاقيات سرية صارمة.' },
      ...
      { title: '8. التحديثات على السياسة', body: 'قد نقوم بتحديث سياسة الخصوصية من وقت لآخر. سنخطّرك بأي تفييرات جوهرية عبر البريد الإلكتروني أو إشعار داخل المنصة.' },
    ],
  },
  terms: {
    title: 'الشروط والأحكام',
    icon: FileText,
    intro: 'باستخدامك لمنصة توافق، فإنك توافق على الشروط والأحكام التالية. يرجى قراءةرها بعناية.',
    sections: [
      ...
      { title: '3. استخدام المنصة', body: 'المنصة مخصّصة للزواج الشرعي والعلاقات الجادة فقط. يُمنع استخدامها لأي أقراض أو غير قانونية أو غير أخلاقية.' },
      ...
      { title: '5. المحتوى', body: 'أنت مسؤول عن المحتوى الذي تنشره. يُمنع نشر محتوى مسيء، مضلّل، أو ينتهك حقوق الآخرين. نحتفظ بحق حذف أي محتوى مخالف.' },
      ...
      <div className="bg-navy-gradient relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 py-10 sm:py-14 text-center">
          <div className="inline-flex items-center justify-center w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-gold-300/15 mb-4">
            <Icon className="w-6 h-6 sm:w-7 sm:h-7 text-gold-300" />
          </div>
          <h1 className="font-cairo font-extrabold text-2xl sm:text-3xl md:text-4xl text-white">{data.title}</h1>
          <p className="mt-3 text-cream-200/80 font-tajawal max-w-xl mx-auto text-sm sm:text-base">آخر تحديث: 1 يناير 2025</p>
        </div>
      </div>

      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        <div className="bg-white rounded-3xl shadow-soft border border-cream-200/60 p-5 sm:p-10">
          <div className="bg-gold-300/10 rounded-2xl p-4 sm:p-5 mb-6 sm:mb-8 flex items-start gap-3 border border-gold-500/20">
            <ShieldCheck className="w-5 h-5 sm:w-6 sm:h-6 text-gold-600 flex-shrink-0 mt-0.5" />
            <p className="text-navy-700 font-tajawal leading-relaxed text-sm sm:text-base">{data.intro}</p>
          </div>

          <div className="space-y-6 sm:space-y-8">
            {data.sections.map((section, i) => (
              <div key={i}>
                <h2 className="font-cairo font-bold text-lg sm:text-xl text-navy-900 mb-2">{section.title}</h2>
                <p className="text-navy-600 font-tajawal leading-loose text-sm sm:text-base">{section.body}</p>
              </div>
            ))}
          </div>
        </div>
