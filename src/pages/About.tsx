    ...
    <div className="bg-cream-50">
      ...
      <section className="relative bg-navy-gradient overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 py-16 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="mt-5 text-lg text-cream-200/80 font-tajawal leading-relaxed max-w-2xl mx-auto">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {STATS.map((s) => (
            <div key={s.label} className="bg-white rounded-3xl p-6 text-center shadow-soft border border-cream-200/60">
              <div className="font-cairo font-extrabold text-3xl text-gradient-gold">{s.value}</div>
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 py-12">
        <div className="grid md:grid-cols-2 gap-6">
          <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="bg-white rounded-3xl p-8 shadow-soft border border-cream-200/60">
            <div className="w-14 h-14 rounded-2xl bg-gold-300/15 flex items-center justify-center mb-4">
              <Target className="w-7 h-7 text-gold-600" />
            </div>
            <h2 className="font-cairo font-extrabold text-2xl text-navy-900 mb-3">رسالتنا</h2>
            <p className="text-navy-600 font-tajawal leading-relaxed">
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="bg-white rounded-3xl p-8 shadow-soft border border-cream-200/60">
            ...
            <h2 className="font-cairo font-extrabold text-2xl text-navy-900 mb-3">رؤيتنا</h2>
            <p className="text-navy-600 font-tajawal leading-relaxed">
      ...
      <section className="py-16 bg-cream-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {values.map((v, i) => (
              <motion.div key={v.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="bg-white rounded-3xl p-6 text-center shadow-soft border border-cream-200/60">
                ...
                <h3 className="font-cairo font-bold text-navy-900 text-lg mb-2">{v.title}</h3>
      ...
      {/* Team */}
      <section className="py-16 bg-white border-t border-cream-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="mt-12 grid sm:grid-cols-3 gap-6">
            {TEAM.map((member, i) => (
              <motion.div key={member.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="bg-white rounded-3xl overflow-hidden shadow-soft border border-cream-200/60 text-center">
                <img src={getAvatar(member.gender)} alt={member.name} className="w-full aspect-square object-contain bg-cream-100 p-8" />
                <div className="p-5">
                  <h3 className="font-cairo font-bold text-navy-900 text-lg">{member.name}</h3>
      ...
      {(socialSettings.showWhatsapp || socialSettings.showTwitter || socialSettings.showInstagram || socialSettings.showFacebook) && (
        <section className="py-16 bg-amber-50/40 border-t border-b border-amber-200/40 text-center">
          <div className="max-w-3xl mx-auto px-4 sm:px-6">
            <h2 className="font-cairo font-extrabold text-2xl text-navy-900 mb-2">الدعم الفني وخدمة العملاء</h2>
            <p className="text-sm text-navy-600 font-tajawal mb-8 leading-relaxed">
              ...
              <div className="flex items-center gap-3">
                {socialSettings.showTwitter && socialSettings.twitter && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-12 h-12 rounded-2xl bg-slate-900 hover:bg-slate-800 text-white flex items-center justify-center transition-colors"
                    title="تويتر"
                  >
                ...
                {socialSettings.showInstagram && socialSettings.instagram && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-amber-500 via-rose-500 to-indigo-500 hover:opacity-95 text-white flex items-center justify-center transition-colors"
                    title="إنستغرام"
                  >
                ...
                {socialSettings.showFacebook && socialSettings.facebook && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-12 h-12 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white flex items-center justify-center transition-colors"
                    title="فيسبوك"
                  >
