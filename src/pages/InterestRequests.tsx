...
export default function InterestRequests() {
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F5] dark:bg-navy-950 text-navy-950 font-sans selection:bg-amber-100 selection:text-amber-900" dir="rtl">
        ...
        <div className="max-w-3xl mx-auto px-4 relative z-10">
          ...
          {!loading && (
            <div className="grid grid-cols-3 gap-2 sm:gap-2.5 mt-6 sm:mt-8">
              <StatChip active={stats.respond > 0} value={stats.respond} label="تحتاج ردّك الفوري" icon={UserCheck} tone="gold" onClick={() => { setTab('active'); setSubFilter('all'); }} />
              <StatChip active={stats.pay > 0} value={stats.pay} label="تحتاج سداد جدية" icon={CreditCard} tone="rose" onClick={() => { setTab('active'); setSubFilter('all'); }} />
              <StatChip active={false} value={stats.active} label="رحلات نشطة جارية" icon={Heart} tone="emerald" onClick={() => { setTab('active'); setSubFilter('all'); }} />
            </div>
          )}
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 -mt-6 pb-20 relative z-20">
            ...
            return (
              <button key={t.key} onClick={() => setTab(t.key)}
                className={`flex-1 flex items-center justify-center gap-1.5 sm:gap-2 py-2.5 sm:py-3 rounded-xl font-cairo font-bold text-[11px] sm:text-sm transition-all duration-300 cursor-pointer
                  ${active ? 'bg-slate-900 dark:bg-amber-500 text-white dark:text-navy-950 shadow-[0_4px_12px_rgba(15,23,42,0.12)]' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50 dark:text-slate-400 dark:hover:text-cream-50 dark:hover:bg-navy-800'}`}>
                <Icon className={`w-3.5 h-3.5 sm:w-4 sm:h-4 flex-shrink-0 ${active ? 'text-amber-400 dark:text-navy-950' : 'text-slate-400'}`} />
                <span className="truncate">{t.label}</span>
                {t.count > 0 && (
                  <span className={`px-1.5 sm:px-2 py-0.5 rounded-lg text-[10px] font-bold transition-colors flex-shrink-0 ${active ? 'bg-amber-400 dark:bg-navy-950 text-slate-950 dark:text-navy-950 font-black' : 'bg-slate-100 dark:bg-navy-800 text-slate-700 dark:text-cream-200'}`}>{t.count}</span>
                )}
              </button>
            );
          })}
        ...
        <div className="flex flex-col sm:flex-row gap-3 mt-4 items-stretch sm:items-center justify-between">
          {tab === 'active' && buckets.active.length > 0 ? (
            <div className="flex gap-1.5 bg-slate-100/60 dark:bg-navy-900/60 p-1 rounded-xl border border-slate-200/50 dark:border-navy-800 self-start overflow-x-auto max-w-full">
              {([
                ...
                <button key={chip.key} onClick={() => setSubFilter(chip.key)}
                  className={`px-3 sm:px-4 py-1.5 rounded-lg text-[11px] sm:text-xs font-cairo font-extrabold transition-all duration-200 cursor-pointer whitespace-nowrap
                    ${subFilter === chip.key ? 'bg-white dark:bg-navy-950 text-slate-950 dark:text-cream-50 shadow-sm' : 'text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-cream-200'}`}>
                  {chip.label}
                </button>
              ))}
            </div>
          ...
          <div className="relative flex-1 max-w-sm w-full sm:w-auto">
            ...
            <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="ابحث باسم الشريك أو المدينة..."
            ...
            {query && (
              <button onClick={() => setQuery('')} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 cursor-pointer">
                <X className="w-4 h-4" />
              </button>
            )}
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 mt-4 bg-white dark:bg-navy-900 p-3 rounded-2xl border border-slate-200/80 dark:border-navy-800 shadow-sm">
          <div className="flex items-center gap-2 flex-wrap">
            <button onClick={() => { const nextMode = !isSelectionMode; setIsSelectionMode(nextMode); if (!nextMode) setSelectedIds([]); }}
              className={`px-3.5 sm:px-4 py-2 rounded-xl text-[11px] sm:text-xs font-cairo font-bold flex items-center gap-2 transition-all cursor-pointer ${isSelectionMode ? 'bg-amber-500 text-navy-950 font-black shadow-md' : 'bg-slate-100 dark:bg-navy-800 text-slate-700 dark:text-cream-100 hover:bg-slate-200 dark:hover:bg-navy-700'}`}>
            ...
            {isSelectionMode && list.length > 0 && (
              <button onClick={toggleSelectAll}
                ...
                <span>{list.length > 0 && list.every((r) => selectedIds.includes(r.id)) ? 'إلغاء تحديد الجميع' : 'تحديد الجميع'}</span>
              </button>
            )}
          </div>
          {isSelectionMode && (
        ...
        <div className="mt-5 space-y-5">
          ...
          {!loading && error && (
            <div className="flex flex-col items-center justify-center py-16 text-center bg-white border border-rose-100 rounded-[2rem] p-6 shadow-sm">
              ...
              <button onClick={refresh} className="font-cairo text-xs font-bold text-amber-600 hover:text-amber-700 bg-amber-50 px-5 py-2.5 rounded-xl border border-amber-200/50 transition-colors cursor-pointer">إعادة المحاولة والاتصال</button>
            </div>
          )}
          {!loading && !error && list.length === 0 && (<EmptyState tab={tab} />)}
          {!loading && !error && (
            <AnimatePresence mode="popLayout">
              {list.map((req, idx) => (
                <RequestCard
                  key={(req?.id !== undefined && req?.id !== null && !Number.isNaN(Number(req.id))) ? `req-${req.id}-${idx}` : `req-idx-${idx}`}
                  req={req} activeId={activeId} getMember={getMember} busy={actionLoading === req.id} urgent={needsAction(req)}
                  unseen={isRequestUnseen(req.id, req.updated_at)} isSelectionMode={isSelectionMode} isSelected={selectedIds.includes(req.id)}
                  onToggleSelect={() => toggleSelectReq(req.id)} onAccept={() => onAccept(req)}
                  onDecline={() => { setDeclineModal(req); setDeclineReason(''); }} onPay={() => { setPayModal(req); setPledge(false); }}
                  onCoord={() => setCoordModal(req)} onResult={() => { setResultModal(req); setResultChoice('success'); setResultNote(''); }}
      ...
      <AcceptedCelebrationModal open={!!acceptModal} memberName={acceptModal?.name || ''} onClose={() => setAcceptModal(null)}
        onInquiry={() => { const r = acceptModal?.req; setAcceptModal(null); if (r) navigate(`/journey/${r.id}?tab=inquiry`); }}
        onOpenJourney={() => { const r = acceptModal?.req; setAcceptModal(null); if (r) navigate(`/journey/${r.id}`); }}
        onProceed={() => { const r = acceptModal?.req; setAcceptModal(null); if (r) navigate(`/journey/${r.id}?tab=deposit`); }} />
      ...
      <Modal open={!!declineModal} onClose={() => setDeclineModal(null)} title="الاعتذار عن الطلب">
        ...
        <div className="space-y-2">
          {DECLINE_REASONS.map((reason) => (
            <label key={reason} className={`flex items-center gap-2.5 p-3 rounded-xl border cursor-pointer transition-all ${declineReason === reason ? 'border-rose-300 bg-rose-50' : 'border-cream-200 hover:bg-cream-50'}`}>
        ...
        {declineReason === DECLINE_REASONS[DECLINE_REASONS.length - 1] && (
          <textarea value={declineCustom} onChange={(e) => setDeclineCustom(e.target.value)} placeholder="اكتب التفاصيل..." rows={3}
        ...
        <button onClick={onDecline} disabled={actionLoading !== null}
          ...
          {actionLoading !== null ? <Loader2 className="w-4 h-4 animate-spin" /> : null} تأكيد الاعتذار
        </button>
      </Modal>
      ...
      <Modal open={!!cancelModal} onClose={() => setCancelModal(null)} title="إلغاء الطلب">
        ...
        <div className="space-y-2">
          {CANCEL_REASONS.map((reason) => (
            <label key={reason} className={`flex items-center gap-2.5 p-3 rounded-xl border cursor-pointer transition-all ${cancelReason === reason ? 'border-rose-300 bg-rose-50' : 'border-cream-200 hover:bg-cream-50'}`}>
        ...
        {cancelReason === CANCEL_REASONS[CANCEL_REASONS.length - 1] && (
          <textarea value={cancelCustom} onChange={(e) => setCancelCustom(e.target.value)} placeholder="اكتب سبب الإلغاء..." rows={3}
            className="w-full mt-3 bg-white border border-cream-200 rounded-xl p-3 text-sm font-cairo focus:outline-none focus:ring-2 focus:ring-rose-200" />
        )}
        <div className="flex gap-2 mt-4">
          <button onClick={() => setCancelModal(null)} className="flex-1 bg-cream-100 text-navy-600 font-cairo font-bold py-3.5 rounded-2xl hover:bg-cream-200 transition-all">تراجع</button>
          <button onClick={onCancel} disabled={actionLoading !== null}
            ...
            {actionLoading !== null ? <Loader2 className="w-4 h-4 animate-spin" /> : <Ban className="w-4 h-4" />} تأكيد الإلغاء
          </button>
        </div>
      </Modal>
        ...
        {payModal && (
          <div>
            ...
            <label className="flex items-start gap-2.5 mt-4 p-3 rounded-xl bg-cream-50 border border-cream-200 cursor-pointer">
              ...
              <span className="text-xs font-cairo text-navy-600 leading-relaxed">أتعهّد أمام الله بأن طلبي هذا بنية الزواج الحقيقي والجاد، وأوافق على شروط المنصة.</span>
            </label>
            <button onClick={onPay} disabled={!pledge || actionLoading !== null}
              ...
              {actionLoading !== null ? <Loader2 className="w-4 h-4 animate-spin" /> : <ShieldCheck className="w-5 h-5" />} سداد رسوم الجدية
            </button>
          </div>
        )}
      </Modal>
        ...
        {coordModal && (
          <div className="space-y-3">
            ...
            <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-3 text-xs font-cairo text-navy-600 leading-relaxed">
              <Info className="w-4 h-4 text-indigo-500 inline ml-1" /> عند تأكيد الموعد وإتمام التوافق الأولي، انتقل لمرحلة تبادل القنوات الرسمية.
            </div>
            {stageOf(coordModal) === 'coordination' && coordModal.meeting_date && (
              <button onClick={() => onAdvanceIntro(coordModal)} disabled={actionLoading !== null}
                ...
                {actionLoading !== null ? <Loader2 className="w-4 h-4 animate-spin" /> : null} المتابعة لمرحلة التوافق
              </button>
            )}
          </div>
        )}
      ...
      <Modal open={!!resultModal} onClose={() => setResultModal(null)} title="تسجيل نتيجة التوافق">
        <div className="grid grid-cols-2 gap-3 mb-4">
          <button onClick={() => setResultChoice('success')} className={`p-4 rounded-2xl border-2 text-center transition-all ${resultChoice === 'success' ? 'border-emerald-400 bg-emerald-50' : 'border-cream-200'}`}>
            <span className="text-2xl block mb-1">💚</span>
            <span className="font-cairo font-bold text-sm text-emerald-700">توافق مبارك</span>
          </button>
          <button onClick={() => setResultChoice('failed')} className={`p-4 rounded-2xl border-2 text-center transition-all ${resultChoice === 'failed' ? 'border-slate-400 bg-slate-50' : 'border-cream-200'}`}>
            <span className="text-2xl block mb-1">🤝</span>
        ...
        <button onClick={onRecordResult} disabled={actionLoading !== null}
          ...
          {actionLoading !== null ? <Loader2 className="w-4 h-4 animate-spin" /> : null} تسجيل النتيجة وإنهاء الرحلة
        </button>
      </Modal>
        ...
        {isSelectionMode && selectedIds.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 80, x: '-50%' }} animate={{ opacity: 1, y: 0, x: '-50%' }} exit={{ opacity: 0, y: 80, x: '-50%' }}
            ...
            <div className="flex items-center gap-2.5">
              <span className="w-8 h-8 rounded-xl bg-gradient-to-tr from-amber-400 to-amber-500 text-navy-950 font-black font-cairo text-sm flex items-center justify-center shadow-sm">{selectedIds.length}</span>
              <div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <button onClick={handleBulkMarkSeen} className="px-3 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-white text-xs font-cairo font-bold flex items-center gap-1.5 transition-all cursor-pointer border border-white/10" title="تعليم الرحلات المحددة كـ تمت مشاهدتها">
                <CheckCircle2 className="w-4 h-4 text-emerald-400" /><span>تعليم كمقروء</span>
              </button>
              <button onClick={() => setConfirmBulkCancel(true)} className="px-3 py-2 rounded-xl bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 text-xs font-cairo font-bold flex items-center gap-1.5 border border-amber-500/30 transition-all cursor-pointer">
                <Ban className="w-4 h-4" /><span>إلغاء المحددة</span>
              </button>
              <button onClick={() => setConfirmBulkDelete(true)} className="px-3.5 py-2 rounded-xl bg-gradient-to-r from-rose-600 to-rose-700 hover:from-rose-700 hover:to-rose-800 text-white text-xs font-cairo font-extrabold flex items-center gap-1.5 transition-all cursor-pointer shadow-md">
                <Trash2 className="w-4 h-4" /><span>حذف المحددة</span>
              </button>
              <button onClick={() => setSelectedIds([])} className="p-2 text-slate-400 hover:text-white rounded-xl hover:bg-white/10 transition-colors cursor-pointer" title="تفريغ التحديد">
      ...
      <Modal open={confirmBulkDelete} onClose={() => setConfirmBulkDelete(false)} title="حذف الرحلات المحددة نهائياً">
        <div className="space-y-4">
          <div className="p-4 bg-rose-50 border border-rose-200 rounded-2xl flex items-start gap-3">
            ...
            <div>
              <p className="font-cairo font-extrabold text-sm text-rose-900">هل أنت تأكد من إزالة وحذف {selectedIds.length} من طلبات الرحلات المحددة؟</p>
              <p className="font-cairo text-xs text-rose-700 mt-1 leading-relaxed">سيتم مسح هذه الرحلات وإزالتها تماماً من قائمتك وسجلك المحلي. لا يمكن التراجع عن هذا الإجراء.</p>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 pt-2">
            <button onClick={handleBulkDelete} className="flex-1 bg-rose-600 hover:bg-rose-700 text-white font-cairo font-extrabold py-3 rounded-xl transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer">
              <Trash2 className="w-4 h-4" /><span>تأكيد حذف {selectedIds.length} رحلة</span>
            </button>
            <button onClick={() => setConfirmBulkDelete(false)} className="sm:px-5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-cairo font-bold py-3 rounded-xl transition-all cursor-pointer">إلغاء</button>
          </div>
        </div>
      </Modal>
      ...
      <Modal open={confirmBulkCancel} onClose={() => setConfirmBulkCancel(false)} title="إلغاء الرحلات المحددة">
        <div className="space-y-4">
          <p className="font-cairo text-sm text-slate-700 dark:text-cream-200 font-bold">سيتم إلغاء {selectedIds.length} رحلة من رحلات التوافق المحددة. يرجى اختيار أو كتابة سبب الإلغاء الجماعي:</p>
          <input value={bulkCancelReason} onChange={(e) => setBulkCancelReason(e.target.value)} placeholder="سبب الإلغاء الجماعي (مثال: عدم الرغبة بالمتابعة حالياً...)"
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 pt-2">
            <button onClick={handleBulkCancel} className="flex-1 bg-amber-500 hover:bg-amber-600 text-navy-950 font-cairo font-extrabold py-3 rounded-xl transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer">
              <Ban className="w-4 h-4" /><span>تأكيد إلغاء {selectedIds.length} رحلة</span>
            </button>
            <button onClick={() => setConfirmBulkCancel(false)} className="sm:px-5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-cairo font-bold py-3 rounded-xl transition-all cursor-pointer">تراجع</button>
          </div>
        </div>
      </Modal>
      ...
      <AnimatePresence>
        {toast && (
          <motion.div initial={{ opacity: 0, y: 50, x: '-50%' }} animate={{ opacity: 1, y: 0, x: '-50%' }} exit={{ opacity: 0, y: 50, x: '-50%' }}
            className={`fixed bottom-6 left-1/2 z-[100] px-5 py-3 rounded-2xl shadow-2xl font-cairo font-bold text-sm text-white max-w-[90%] ${toast.type === 'success' ? 'bg-emerald-600' : toast.type === 'error' ? 'bg-rose-deep' : 'bg-navy-800'}`}>
            {toast.text}
          </motion.div>
        )}
      </AnimatePresence>
...
function RequestCard({
  ...
  key?: any; req: ApiRequest; activeId: string; getMember: (id: string) => any; busy: boolean; urgent: boolean; unseen: boolean;
  isSelectionMode?: boolean; isSelected?: boolean; onToggleSelect?: () => void; onAccept: () => void | Promise<void>; onDecline: () => void;
  onPay: () => void; onCoord: () => any; onResult: () => void; onCancel: () => void;
}) {
  ...
  const other = otherResolved || {
    ...
    age: 28, country: 'السعودية', city: 'الرياض', verified: false, premium: false,
  };
  ...
  const loadTimeline = async () => {
    if (!showTimeline && events.length === 0) { setEvents(await fetchRequestEvents(req.id)); }
    setShowTimeline((v) => !v);
  };
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.98 }}
      ...
      {isSelectionMode && (
        <button onClick={(e) => { e.stopPropagation(); onToggleSelect?.(); }}
          ...
          {isSelected ? <CheckSquare className="w-6 h-6 text-amber-500 fill-amber-100" /> : <Square className="w-6 h-6 text-slate-300 dark:text-navy-600" />}
        </button>
      )}
      {urgent && <div className={`absolute right-0 top-0 bottom-0 w-2.5 ${railColor}`} />}
      {urgent && !isSelectionMode && (
        <div className="absolute top-3 left-3 sm:top-4 sm:left-4 z-10">
          <motion.span animate={{ scale: [1, 1.04, 1], opacity: [0.95, 1, 0.95] }} transition={{ duration: 2, repeat: Infinity }}
            className={`inline-flex items-center gap-1.5 text-[9px] sm:text-[10px] font-cairo font-black px-2.5 sm:px-3.5 py-1 sm:py-1.5 rounded-full text-white shadow-md ${urgency === 'pay' ? 'bg-gradient-to-r from-rose-500 to-rose-600' : 'bg-gradient-to-r from-amber-500 to-amber-600'}`}>
      ...
      <div className="p-4 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3 sm:gap-4">
          <Link to={`/member/${other.id}`} className="flex-shrink-0 relative group block cursor-pointer">
            ...
            {unseen && (<span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-rose-500 border-2 border-white animate-pulse" />)}
          </Link>
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <Link to={`/member/${other.id}`} className="font-cairo font-black text-base sm:text-lg text-slate-900 dark:text-cream-50 hover:text-amber-600 dark:hover:text-amber-400 transition-colors cursor-pointer leading-tight">{other.nickname}</Link>
              {isOtherDeleted && (<span className="text-[10px] font-cairo font-black text-rose-700 dark:text-rose-400 bg-rose-100 dark:bg-rose-950/40 border border-rose-300 dark:border-rose-900 px-2.5 py-0.5 rounded-full">⚠️ حساب مُحذوف</span>)}
              {isOtherBanned && (<span className="text-[10px] font-cairo font-black text-rose-700 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-900 px-2.5 py-0.5 rounded-full">⛔ حساب محظور</span>)}
              {isOtherSuspended && (<span className="text-[10px] font-cairo font-black text-orange-700 dark:text-orange-400 bg-orange-50 dark:bg-orange-950/30 border border-orange-200 dark:border-orange-900 px-2.5 py-0.5 rounded-full">⚠️ حساب موقوف</span>)}
              ...
              {unseen && (<span className="text-[9px] font-cairo font-black text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/30 border border-rose-100 dark:border-rose-900 px-2 py-0.5 rounded-full">تحديث</span>)}
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-300 font-cairo mt-1.5">
              <MapPin className="w-3.5 h-3.5 text-slate-400" /> <span>يقيم في {other.city}، {other.country}</span>
            </div>
        ...
        <div className="px-4 sm:px-6 pb-4">
          <div className="relative bg-amber-50/40 dark:bg-amber-950/10 border border-amber-200/40 dark:border-amber-900/20 rounded-2xl p-4 shadow-inner">
            <div className="absolute right-3.5 -top-2.5 bg-white dark:bg-navy-950 px-2.5 py-0.5 rounded-full border border-amber-200/50 dark:border-amber-900/40 text-[9px] font-bold text-amber-800 dark:text-amber-400 font-cairo flex items-center gap-1">
              <Sparkles className="w-2.5 h-2.5 text-amber-500" /> رسالة الاهتمام من الشريك
            </div>
            <p className="text-xs sm:text-sm text-slate-700 dark:text-slate-200 font-cairo leading-relaxed italic">" {req.message} "</p>
          </div>
        </div>
      )}
      <div className="px-4 sm:px-6 pb-4"><RequestStatusPanel summary={summary} compact /></div>
      <div className="px-4 sm:px-6 pb-4"><JourneyTimeline stage={stage} isSender={isSender} /></div>
      {stage === 'declined' && req.decline_reason && (
        <div className="mx-4 sm:mx-6 mb-4 text-xs font-cairo text-rose-700 bg-rose-50/50 border border-rose-100 rounded-xl p-3 flex items-start gap-1.5">
          ...
          <div><span className="font-bold">ملاحظة الاعتذار:</span> {req.decline_reason}</div>
        </div>
      )}
      {stage === 'completed' && req.evaluation_note && (
        <div className="mx-4 sm:mx-6 mb-4 text-xs font-cairo text-emerald-700 bg-emerald-50/50 border border-emerald-100 rounded-xl p-3 flex items-center gap-1.5">
          <span>{req.evaluation_result === 'failed' ? '🤝 ' : '💚 '}</span><span>{req.evaluation_note}</span>
        </div>
      )}
      {(stage === 'coordination' || stage === 'sharia_viewing' || stage === 'engagement') && req.meeting_date && (
        <div className="mx-4 sm:mx-6 mb-4 text-xs font-cairo text-indigo-700 bg-indigo-50 border border-indigo-100 rounded-xl p-3 flex items-center gap-2">
          <CalendarClock className="w-4 h-4 text-indigo-500 animate-pulse" /> <span className="font-bold">موعد اللقاء الشرعي المنسق:</span> {req.meeting_date}
        </div>
      )}
      {stage === 'seriousness' && (
      ...
      {isOtherUnavailable && (
        <div className="mx-4 sm:mx-6 mb-4 bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-900 text-rose-900 dark:text-rose-200 rounded-2xl p-4 shadow-sm flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="font-cairo text-xs text-rose-700 dark:text-rose-300 mt-1 leading-relaxed">تعذّر استكمال إجراءات هذه الرحلة لأن حساب الطرف الآخر غير متاح. يمكنك إلغاء الطلب لإزالته من القائمة النشطة.</p>
          </div>
        </div>
      )}
      <div className="px-4 sm:px-6 pb-5 pt-3.5 border-t border-slate-100 dark:border-navy-800 bg-slate-50/30 dark:bg-navy-950/30">
        {isOtherUnavailable ? (
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-rose-50/70 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-900 p-3.5 rounded-2xl">
            ...
            <button onClick={onCancel} disabled={busy}
              ...
              <Ban className="w-4 h-4" /> إلغاء وإزالة الطلب
            </button>
          </div>
        ) : (
          <PrimaryActionBar stage={stage} action={action} busy={busy} requestId={req.id} onAccept={onAccept} onDecline={onDecline} onPay={onPay} onCoord={onCoord} onResult={onResult} />
        )}
        {action.type === 'accept_decline' && (
          <p className="text-[10px] text-slate-500 font-cairo text-center mt-2.5 leading-relaxed flex items-center justify-center gap-1">
            <ShieldCheck className="w-3.5 h-3.5 text-emerald-500 flex-shrink-0" /> <span>لن تتم مشاركة أي أرقام تواصل أو بيانات حساسة؛ فقط سيتم فتح مسار التعارف المبدئي بأسئلة محددة.</span>
          </p>
        )}
        {action.type === 'pay_deposit' && (
          <p className="text-[10px] text-rose-700 font-cairo text-center mt-2.5 leading-relaxed font-bold bg-rose-50 border border-rose-200/40 rounded-xl py-1.5 px-3 flex items-center justify-center gap-1.5">
            <Info className="w-3.5 h-3.5 text-rose-500 flex-shrink-0" /> <span>تنبيه هام: رسوم تأكيد الجدية وجميع الرسوم المدفوعة غير مستردة نهائياً تحت أي ظرف.</span>
          </p>
        )}
        {stage === 'sharia_viewing' && (
          <p className="text-[10px] text-indigo-700 font-cairo text-center mt-2.5 leading-relaxed bg-indigo-50 border border-indigo-100/50 rounded-xl py-1.5 px-3 flex items-center justify-center gap-1.5">
            <CalendarClock className="w-3.5 h-3.5 text-indigo-500 flex-shrink-0" /> <span>بعد اللقاء الشرعي، حدد "توافق مبارك" لتوثيق الملكة أو "لم يكتمل" للاعتذار وإغلاق الطلب بلطف.</span>
          </p>
        )}
        <div className="grid grid-cols-2 gap-2.5 mt-3.5">
          {!isTerminal(stage) && (
            <Link to={`/journey/${req.id}`} className="flex-1 flex items-center justify-center gap-2 font-cairo font-bold text-[11px] sm:text-xs text-amber-800 bg-amber-500/10 hover:bg-amber-500/15 rounded-xl py-3 transition-all cursor-pointer border border-amber-300/20 shadow-sm">
        ...
        <button onClick={loadTimeline} className="mt-3 w-full flex items-center justify-center gap-1.5 text-[10px] font-cairo font-bold text-slate-400 hover:text-slate-600 transition-colors py-1 cursor-pointer">
          ...
          {showTimeline && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
              <div className="pt-3 space-y-2 border-t border-slate-200/40 mt-2">
                {events.length === 0 && (<p className="text-[10px] text-slate-400 font-cairo text-center py-2">لم يتم تسجيل أي أحداث للرحلة حتى الآن</p>)}
                {events.map((ev, idx) => (
    ...
    <div className="bg-white rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(15,23,42,0.02)] overflow-hidden transition-all duration-300">
      <button onClick={() => setOpen((v) => !v)} className="w-full flex items-center justify-between px-5 py-4 cursor-pointer hover:bg-slate-50/50 transition-colors">
        <span className="flex items-center gap-2.5 font-cairo font-black text-xs sm:text-sm text-slate-800">
          ...
          <span>كيف تعمل رحلة التوافق للزواج الشرعي ا